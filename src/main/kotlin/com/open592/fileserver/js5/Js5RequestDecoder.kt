...
class Js5RequestDecoder : ByteToMessageDecoder() {
  ...
  private fun decodeRequestGroupPacket(input: ByteBuf, isPrefetch: Boolean): Js5Message.RequestGroup {
  ...
  private fun decodeRequestConnectionDisconnectPacket(input: ByteBuf): Js5Message.RequestConnectionDisconnect {
    // Skip padding bytes
    input.skipBytes(3)
  ...
  private fun decodeExchangeObfuscationKeyPacket(input: ByteBuf): Js5Message.ExchangeObfuscationKey {
  ...
  private fun decodeInitializeJs5RemoteConnectionPacket(input: ByteBuf): Js5Message.InitializeJs5RemoteConnection {
    val build = input.readUnsignedByte().toInt()
